...
namespace _3.PL.Views
{
    public partial class FrmThongKe : Form
    {
        ...
        public FrmThongKe()
        {
            ...
            LoadData();
            LoadDataCbxThang();
            lbl_SoHDTaiQuay.Text = _IHoaDonService.GetallHoadon().Where(c => c.NgayGiao == null).Count().ToString();
            lbl_SoHDDangGiao.Text = _IHoaDonService.GetallHoadon().Where(c => c.NgayGiao !=null).Count().ToString();
            LoadDataNam();
        ...
        private void LoadData()
        {
            ...
            foreach (var x in _IHoaDonCTService.GetAllHoaDonCT().OrderBy(c => c.IdHoaDon).ToList())
            {
                ...
                dgrid_ThongKe.Rows.Add(x.IdHoaDon, x1.Ma, x.SoLuong, x.SoLuong * x2.GiaBan, x1.NgayGiao == null ? "Mua tại quầy" : "Giao hàng", x1.NgayThanhToan);
            }
        }
        private void LoadDataTimKiem()
        {
            ...
            dgrid_ThongKe.Columns[5].Name = "Ngày TT";
            dgrid_ThongKe.Rows.Clear();
            var ok = _IHoaDonCTService.GetViewHoaDonCT().Where(c => c.HoaDon.NgayThanhToan >= date_NgayBD.Value && c.HoaDon.NgayThanhToan <= date_NgayKT.Value);
            foreach (var x in ok.OrderBy(c => c.HoaDon.Id).ToList())
            {
                ...
                dgrid_ThongKe.Rows.Add(x.HoaDon.Id, x.HoaDon.Ma, x.HoaDonChiTiet.SoLuong, x.HoaDonChiTiet.SoLuong * x.ChiTietGiay.GiaBan, x.HoaDon.NgayGiao == null ? "Mua tại quầy" : "Giao hàng", x.HoaDon.NgayThanhToan);
            }
        }
        ...
        private void cmb_LocThang_SelectedValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                lbl_DoanhThuHomNay.Text = Convert.ToString(_IHoaDonService.GetallHoadon().Where(c => c.NgayThanhToan.Value.ToString().Substring(0, 2) == cmb_LocThang.Text.ToString()).Sum(c => c.TongTien));
            }
            catch (Exception ex)
            {
                MessageBox.Show(Convert.ToString(ex.Message), "Liên Hệ Với KaiSan");
        ...
        private void cmb_LocNam_SelectedValueChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                lbl_DoanhThuHomNay.Text = Convert.ToString(_IHoaDonService.GetallHoadon().Where(c => c.NgayThanhToan.Value.ToString().Substring(6, 4) == cmb_LocNam.Text.ToString()).Sum(c => c.TongTien));
            }
            catch (Exception ex)
            {
                MessageBox.Show(Convert.ToString(ex.Message), "Liên Hệ Với KaiSan");
